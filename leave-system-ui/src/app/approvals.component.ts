  ...
  template: `
    ...
    <div style="padding:24px;font-family:Arial,sans-serif;background:#f3f4f6;min-height:100vh;">
      ...
      <div *ngIf="selectedIds.length > 0"
        style="background:#1e40af;color:white;padding:14px 20px;border-radius:10px;margin-bottom:16px;display:flex;align-items:center;gap:14px;flex-wrap:wrap;">
      ...
      <div style="background:white;padding:20px;border-radius:12px;box-shadow:0 2px 8px rgba(0,0,0,.08);">
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:16px;flex-wrap:wrap;gap:10px;">
          ...
          <span *ngIf="!loading"
            style="background:#fef3c7;color:#92400e;padding:6px 14px;border-radius:999px;font-size:13px;font-weight:600;">
        ...
        <div *ngIf="error && !loading"
          style="background:#fee2e2;color:#991b1b;padding:14px 16px;border-radius:8px;margin-bottom:12px;">
            ...
            <tbody>
              <ng-container *ngFor="let r of requests">
                <tr [style.background]="isSelected(r.id) ? '#eff6ff' : 'white'">
                  ...
                  <td style="padding:12px;border-bottom:1px solid #f3f4f6;color:#6b7280;max-width:160px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;"
                    [title]="r.reason">{{ r.reason }}</td>
                  <td style="padding:12px;border-bottom:1px solid #f3f4f6;">
                    <div style="display:flex;gap:6px;">
                      <button (click)="approve(r.id)" [disabled]="isProcessing(r.id)"
                        ...
                        [style.opacity]="isProcessing(r.id) ? '0.5' : '1'">
                        {{ isProcessing(r.id) ? '...' : 'Approve' }}
                      </button>
                      <button (click)="startReject(r.id)" [disabled]="isProcessing(r.id)"
                ...
                <tr *ngIf="rejectingId === r.id" style="background:#fff5f5;">
                  <td colspan="7" style="padding:12px 16px;border-bottom:1px solid #f3f4f6;">
                    <div style="display:flex;gap:10px;align-items:center;flex-wrap:wrap;">
                      <input [(ngModel)]="rejectComment" placeholder="Rejection comment (optional)"
                        style="padding:8px 12px;border:1px solid #fca5a5;border-radius:6px;flex:1;min-width:220px;font-size:13px;" />
                      <button (click)="confirmReject(r.id)" [disabled]="isProcessing(r.id)"
                        style="background:#dc2626;color:white;border:none;padding:8px 16px;border-radius:6px;cursor:pointer;font-size:13px;font-weight:600;"
                        [style.opacity]="isProcessing(r.id) ? '0.5' : '1'">
                        {{ isProcessing(r.id) ? 'Rejecting...' : 'Confirm Reject' }}
                      </button>
...
export class ApprovalsComponent implements OnInit {
  ...
  loadRequests() {
    ...
    this.api.getPendingRequests().pipe(
      ...
      next: data => { this.requests = data; this.selectedIds = []; this.processingIds.clear(); },
      error: (err) => this.error = err?.error?.message || 'Could not load requests.'
    });
  }

  isSelected(id: number) { return this.selectedIds.includes(id); }
  isProcessing(id: number) { return this.processingIds.has(id); }
  ...
  approve(id: number) {
    ...
    this.api.approveRequest(id).pipe(
      ...
      next: () => { this.showToast('Request approved.'); this.loadRequests(); },
  ...
  bulkApprove() {
    ...
    this.api.bulkApprove({ requestIds: [...this.selectedIds] }).pipe(
      ...
      next: () => { this.showToast(`${count} request(s) approved.`); this.loadRequests(); },
  ...
  bulkReject() {
    this.bulkLoading = true;
    const count = this.selectedIds.length;
    this.api.bulkReject({ requestIds: [...this.selectedIds], comment: this.bulkRejectComment || null }).pipe(
      finalize(() => this.bulkLoading = false)
    ).subscribe({
      next: () => {
